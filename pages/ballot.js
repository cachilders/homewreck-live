...
function Ballot({ stories = [], voter = {}, protocol, host }) {
  ...
  const handleSelect = ({ target: { id, value } }) =>
    setVotes({ ...votes, [id]: value });
  const options = stories.map(({ id, name }) => (
    <option key={id} value={id} disabled={selected.includes(id.toString())}>
      {name}
    </option>
  ));
  ...
  return (
    <div className="w-full max-w-xs m-auto mt-10">
      ...
      <ImageCard
        subtext={`OK. Listen. You bought ${totalVotes} votes, and you've got ${maxVotes} remaining.`}
      />
      <div className="mt-10">
        <form className="bg-gray-100 border shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          <div className="mb-4">
            ...
            <div className="inline-block relative w-64">
              ...
              >
                {maxVotes &&
                  Array.from(Array(maxVotes).keys()).map((v) => (
          ...
          <div className="flex items-center justify-between">
            <button
              className={`${
                voteDisabled ? 'bg-gray-500' : 'bg-red-500 hover:bg-red-600'
